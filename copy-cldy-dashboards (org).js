...
/* ------------------------------------------------------------
 * TEST: Cross-dashboard widget duplication
 * ---------------------------------------------------------- */
async function testCrossDashboardTabCopy({
  sourceDashboardId,
  sourceTabName,
  ...
  console.log("==================================================");
  console.log("TESTING CROSS-DASHBOARD TAB COPY");
  console.log("==================================================");
  ...
  const sourceDashboard = await httpRequest(
    `${SRC_BASE}/v1/dashboards/${sourceDashboardId}`,
    {
      method: "GET",
      headers: SRC_HEADERS
    }
  ...
  const dashboard = sourceDashboard.result;

  if (!dashboard.tabs || dashboard.tabs.length === 0) {
    throw new Error("Source dashboard has no tabs");
  }
  ...
  const sourceTab = dashboard.tabs.find(
    tab => tab.name === sourceTabName
  );

  if (!sourceTab) {
    throw new Error(
      `Could not find source tab "${sourceTabName}"`
    );
  }

  console.log(`✅ Found source tab: ${sourceTab.name}`);
  console.log(`✅ Source tab ID: ${sourceTab.id}`);

  // ----------------------------------------------------------
  // 3. Get widgets belonging to source tab
  // ----------------------------------------------------------

  const tabWidgets = dashboard.widgets.filter(
    widget => widget.tab_id === sourceTab.id
  );
...
(async () => {
  try {
    await testCrossDashboardTabCopy({
      sourceDashboardId: 835521,
      sourceTabName: "AI",
      destinationDashboardId: 987654,
      newTabName: "Cross Dashboard Duplicate Test"
    });
